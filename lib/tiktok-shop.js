...
const DEFAULT_APP_KEY="6l59apmu4302k";
const DEFAULT_SERVICE_ID="7680117059338225428";
function appKey(){ return text(process.env.TIKTOK_SHOP_APP_KEY||process.env.TIKTOK_APP_KEY||DEFAULT_APP_KEY,300); }
function appSecret(){ return text(process.env.TIKTOK_SHOP_APP_SECRET||process.env.TIKTOK_APP_SECRET,1000); }
function serviceId(){ return text(process.env.TIKTOK_SHOP_SERVICE_ID||process.env.TIKTOK_SERVICE_ID||DEFAULT_SERVICE_ID,300); }
...
export function tiktokShopConfig(){
  const cfg={appKey:appKey(),appSecret:appSecret(),serviceId:serviceId(),enabled:enabled()};
  return {...cfg,missing:[!cfg.appKey&&"TIKTOK_SHOP_APP_KEY",!cfg.appSecret&&"TIKTOK_SHOP_APP_SECRET",!cfg.serviceId&&"TIKTOK_SHOP_SERVICE_ID"].filter(Boolean)};
}
