...
async function recordWarrantyLookup(phoneInput,{status="error",itemCount=0,customerName="",productNames=[]}={}){
  ...
  // Đồng bộ trực tiếp vào hash ngày mà trang quản trị đang đọc.
  await safe(["HINCRBY",`analytics:v5:day:${day}`,"warrantyChecks","1"]);
  await safe(["EXPIRE",`analytics:v5:day:${day}`,String(exp)]);
  ...
  if(status==="found" || status==="not_found" || status==="error"){
    ...
    const field=status==="found" ? "warrantyFound" : status==="not_found" ? "warrantyNotFound" : "warrantyErrors";
    await safe(["HINCRBY",`analytics:v5:day:${day}`,field,"1"]);
    await safe(["EXPIRE",`analytics:v5:day:${day}`,String(exp)]);
  }
  ...
  const entry=JSON.stringify({
    phone:phone,
    phoneFull:phone,
    phoneStorageVersion:2,
    ts:Date.now(),
    status,
    itemCount:Number(itemCount||0),
    customerName:String(customerName||"").trim(),
    productNames:Array.isArray(productNames) ? productNames.map(x=>String(x||"").trim()).filter(Boolean).slice(0,12) : []
  });
...
export default async function handler(req,res){
  ...
  try{
    ...
    await recordWarrantyLookup(req.body?.phone,{
      ...
      productNames:Array.isArray(result?.items)?result.items.map(x=>x?.productName).filter(Boolean):[]
    });
