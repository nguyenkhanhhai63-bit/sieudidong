...
function normalizedModel(str="") {
  return removeVietnameseMarksSimple(str)
    ...
    // V853: máy cũ iPhone có tên biến thể kiểu
    // "iPhone 8 cũ - Đen - 64 - 98% - Pin 8X". Các phần này là thuộc tính,
    // không phải tên model; bỏ chúng trước khi đối chiếu link thông số.
    .replace(/\bcu\b/g, " ")
    .replace(/\b\d{2,3}\s*%/g, " ")
    .replace(/\bpin\s*\d+[a-z]?\b/g, " ")
    .replace(/\b(?:32|64|128|256|512|1024)\b/g, " ")
    .replace(/\bchinh\s*hang\b/g, " ")
    // V913: 4G/5G chỉ là hậu tố mạng, không phải tên model. Ví dụ
    // "OPPO Find X9S Chính hãng" phải khớp link "OPPO Find X9S 5G".
    .replace(/\b(?:4g|5g)\b/g, " ")
...
function resolveManualLink(productName, links) {
  ...
  // V844: chỉ dùng link của model khớp chính xác sau khi chuẩn hóa.
  // Trước đây startsWith/includes làm model tên gần nhau ăn chung link nguồn.
  for (const [model, value] of Object.entries(links || {})) {
    const url = typeof value === "string" ? value : value?.url;
    if (!url) continue;
    const key = normalizedModel(model);
    if (key && key === target) return { model, url };
  }
