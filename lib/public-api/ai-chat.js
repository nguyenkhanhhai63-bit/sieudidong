...
function isWarrantyIntent(message="",history=[]){
  ...
  // Chỉ tin nhắn HIỆN TẠI có ý định bảo hành mới mở luồng bảo hành.
  ...
  // Chỉ dùng lịch sử cho đúng 1 trường hợp:
  // AI vừa hỏi SĐT để tra cứu bảo hành và khách trả lời bằng SĐT.
  ...
  const t=normSearch(lastAssistant.text||"");
  const explicitlyWaitingForWarrantyPhone=
    (t.includes("bao hanh") || t.includes("tra cuu")) &&
    (t.includes("so dien thoai") || t.includes("sdt"));

  return explicitlyWaitingForWarrantyPhone;
}
