...
function lastAssistantAskedWarrantyPhone(history=[]){
  // history từ frontend có thể dùng role assistant/AI tùy phiên bản.
  for(let i=history.length-1;i>=0;i--){
    const x=history[i];
    if(x?.role!=="assistant" && x?.role!=="AI") continue;
    const t=normSearch(x?.text||"");
    return (
      (t.includes("bao hanh") || t.includes("tra cuu")) &&
      (
        t.includes("so dien thoai") ||
        t.includes("sdt") ||
        t.includes("so dt")
      )
    );
...
export default async function handler(req,res){
  ...
  if(followupWarrantyPhone && lastAssistantAskedWarrantyPhone(history)){
