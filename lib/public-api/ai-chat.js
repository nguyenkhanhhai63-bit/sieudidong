...
function resolveConversationProduct(message="",history=[],products=[],semanticTarget=""){
  ...
  const texts=[semanticTarget,message];
  for(let i=history.length-1;i>=0;i--) texts.push(history[i]?.text||"");
  let best=null,bestScore=0,bestRecency=0;
  texts.slice(0,10).forEach((text,idx)=>{
    products.forEach(p=>{
      const base=productMatchScore(text,p);
      if(!base) return;
      const recency=Math.max(0,18-idx*2);
      const score=base+recency;
      if(score>bestScore){best=p;bestScore=score;bestRecency=recency;}
    });
  });
  return bestScore>=16?{...best,_contextScore:bestScore,_recency:bestRecency}:null;
}
...
async function understandCustomerQuestion(message="",history=[],apiKey=""){
  ...
  const sys=[
    ...
    "intent phải là một trong: installment_general, installment_interest, installment_downpayment, installment_monthly, installment_term, stock, price, warranty, recommendation, store_info, policy, repair, tradein, product_info, comparison, order, human_request, other.",
  ...
  try{
    ...
    const allowed=new Set(["installment_general","installment_interest","installment_downpayment","installment_monthly","installment_term","stock","price","warranty","recommendation","store_info","policy","repair","tradein","product_info","comparison","order","human_request","other"]);
...
export default async function handler(req,res){
  ...
  const systemInstruction=[
    ...
    "ICON CẢM XÚC: hãy dùng emoji như một nhân viên chat thật, nhưng chỉ ĐÔI LÚC và đúng cảm xúc. Khoảng 20-30% tin nhắn thân thiện có thể có 1 emoji ở cuối hoặc giữa câu: 🙂 khi thân thiện, 😅 khi nhận nhầm/xin lỗi nhẹ, 👍 khi xác nhận, ❤️ khi cảm ơn, 🔥 khi khen hiệu năng, 🤝 khi chốt hỗ trợ. Nếu đoạn chat đang nghiêm túc hoặc chỉ hỏi dữ liệu thì không cần emoji.",
    "Không dùng emoji trong câu chỉ có giá, tồn kho chính xác, điều khoản bảo hành, thông tin pháp lý hoặc dữ liệu kỹ thuật cần đọc rõ. Không dùng quá 1 emoji trong một bong bóng và tuyệt đối không dùng chuỗi emoji. Tránh lặp cùng một emoji liên tiếp qua nhiều lượt.",
    "Không tự giới thiệu mình là AI, bot, Gemini, mô hình hay trợ lý AI trong câu trả lời cho khách. Nếu khách hỏi trực tiếp đây có phải người thật không, phải trả lời trung thực rằng đây là hệ thống hỗ trợ tự động của Siêu Di Động và có thể chuyển nhân viên khi cần.",
