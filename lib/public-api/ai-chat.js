...
function productMatchScore(text="",product={}){
  ...
  // V864: khóa cả hậu tố dính liền số model: Ace 6 != Ace 6T, Xiaomi 15 != 15T.
  // Trước đây modifier "t" chỉ bắt token rời nên "6t" vẫn có thể ăn điểm từ "6".
  const attachedMods=["pro","max","ultra","plus","mini","turbo","rt","neo","se","t","s"];
  const qWords=q.split(/\s+/).filter(Boolean), nWords=n.split(/\s+/).filter(Boolean);
  for(const nw of nWords){
    const m=nw.match(/^(\d+)(pro|max|ultra|plus|mini|turbo|rt|neo|se|t|s)$/);
    if(m && qWords.includes(m[1]) && !qWords.includes(nw)) score-=70;
  }
  for(const qw of qWords){
    const m=qw.match(/^(\d+)(pro|max|ultra|plus|mini|turbo|rt|neo|se|t|s)$/);
    if(m && nWords.includes(m[1]) && !nWords.includes(qw)) score-=70;
  }
  return score;
}

function explicitModelTokens(text=""){
  const q=normSearch(text);
  // Model dạng chữ+số như X200S, K90, Z10X, 15T... phải khớp nguyên token.
  // Tránh X200S bị đổi thành X200 Pro chỉ vì tên gần giống.
  return q.split(/\s+/).filter(t=>/[a-z]/.test(t)&&/\d/.test(t)&&t.length>=3);
}

function bestExplicitProduct(text="",products=[]){
  const exactModels=explicitModelTokens(text);
  let best=null,score=0;
  for(const p of products){
    const productName=normSearch(p?.name||"");
    const productTokens=new Set(productName.split(/\s+/).filter(Boolean));
    // Nếu khách gõ rõ mã model thì sản phẩm phải chứa đúng mã đó.
    // VD: x200s chỉ match x200s, không match x200 / x200 pro.
    if(exactModels.length && !exactModels.every(t=>productTokens.has(t))) continue;
...
// V864: lấy 2 model gần nhất khách đã thực sự nhắc để xử lý câu "2 con này", "so sánh 2 máy".
function recentConversationProducts(history=[],products=[],limit=2){
  const out=[];
  for(let i=history.length-1;i>=0 && out.length<limit;i--){
    const h=history[i]||{};
    if(historyRole(h)!=="Khách") continue;
    const hits=explicitProductsInMessage(h.text||h.content||h.message||"",products);
    const candidates=hits.length?hits:(()=>{ const x=bestExplicitProduct(h.text||h.content||h.message||"",products); return x?[x.product]:[]; })();
    for(const p of candidates){
      if(!out.some(x=>normSearch(x.name)===normSearch(p.name))) out.push(p);
...
function productPairReferenceQuestion(message=""){
  const q=normSearch(message);
  return /(?:so sanh|khac nhau|con nao|may nao).*(?:2|hai).*(?:con|may)|(?:2|hai).*(?:con|may).*(?:nay|do)|(?:2 con nay|hai con nay|2 may nay|hai may nay)/.test(q);
}
...
function directInstallmentAnswer(settings={},product=null){
  const providers=(Array.isArray(settings.providers)?settings.providers:[])
    .filter(x=>x&&x.enabled!==false&&x.name)
    .map(x=>x.name);
  const productName=clean(product?.name,180);
  const lines=[productName?`${productName} có hỗ trợ trả góp nha b.`:"Có nha b. Shop có hỗ trợ trả góp á."];
  if(providers.length) lines.push(`Shop đang làm qua ${providers.join(" với ")} á.`);
  lines.push("Cọc bao nhiêu với góp mấy tháng thì bên tài chính duyệt theo hồ sơ nha b.");
  // V863: nếu khách đã nêu model ở câu hiện tại hoặc ngữ cảnh thì tuyệt đối không hỏi lại máy nào.
  // Chỉ hỏi model khi toàn bộ hội thoại chưa xác định được sản phẩm.
  if(!productName) lines.push("B đang ngắm máy nào gửi mình coi thử nha.");
  return lines.join("\n");
}
...
export default async function handler(req,res){
  ...
  const comparisonTargets=(questionUnderstanding.intent==="comparison" || productPairReferenceQuestion(message))
    ? recentConversationProducts(history,products,2)
    : [];
  const isComparisonTurn=questionUnderstanding.intent==="comparison" || productPairReferenceQuestion(message);
  ...
  const singleConditionAnswer=isComparisonTurn?"":directSingleProductConditionAnswer(message,history,products,questionUnderstanding.target);
  ...
  const multiConditionAnswer=isComparisonTurn?"":directMultiProductConditionAnswer(message,products);
  ...
  const namedProductAnswer=isComparisonTurn?"":directNamedProductAnswer(message,history,products,questionUnderstanding.target,questionUnderstanding.intent);
  ...
  const priceAnswer=(!isComparisonTurn && questionUnderstanding.intent==="price")
  ...
  if(questionUnderstanding.intent.startsWith("installment_") || isInstallmentIntent(message)){
    ...
    if(!detail){
      return await reply({
        ok:true,
        text:directInstallmentAnswer(installmentSettings,resolveConversationProduct(message,history,products,questionUnderstanding.target)),
  ...
  const colorAnswer=isComparisonTurn?"":directColorAnswer(message,products,history,questionUnderstanding.target);
  if(colorAnswer){
    return await reply({
      ok:true,
      text:colorAnswer,
  ...
  const stockAnswer=(!isComparisonTurn && (questionUnderstanding.intent==="stock" || isStockQuestion(message)))
  ...
  const systemInstruction=[
    ...
    `CẶP MÁY ĐANG SO SÁNH: ${comparisonTargets.length>=2?comparisonTargets.map(x=>x.name).join(" ↔ "):"chưa xác định đủ 2 máy"}.`,
    "Nếu khách nói '2 con này', 'hai máy này', 'con nào hơn' hoặc yêu cầu so sánh mà CẶP MÁY ĐANG SO SÁNH đã đủ 2 model, phải so sánh đúng hai model đó. Không được biến câu so sánh thành trả lời giá/tồn kho của một máy.",
    ...
    "QUY TẮC KHÔNG HỎI LẠI: Trước khi đặt câu hỏi cho khách, phải kiểm tra câu hiện tại và lịch sử hội thoại. Thông tin khách đã cung cấp như model, phiên bản, màu, nhu cầu, ngân sách hoặc hình thức trả góp thì tuyệt đối không hỏi lại. Nếu đã biết model, không được hỏi kiểu 'bạn đang ngắm máy nào'. Chỉ hỏi đúng dữ liệu thực sự còn thiếu và cần thiết để trả lời.",
