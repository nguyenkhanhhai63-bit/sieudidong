...
function isWarrantyIntent(message="",history=[]){
  const current=normSearch(message);
  const recent=normSearch(
    history.slice(-4).map(x=>String(x?.text||"")).join(" ")
  );

  const compact=(current+" "+recent).replace(/\s+/g," ").trim();

  // Hiểu cả viết tắt / không dấu / sai chính tả thường gặp:
  // bh, check bh, kiểm tra bh, bao hanh, bảo hành, còn bh, hết bh...
  ...
  if(directPatterns.some(rx=>rx.test(compact))) return true;

  // Nếu AI vừa yêu cầu SĐT để kiểm tra bảo hành, tin nhắn kế tiếp
  // chỉ có số điện thoại vẫn phải tiếp tục luồng bảo hành.
  const assistantRecentlyAskedWarranty=history.slice(-4).some(x=>{
    if(x?.role!=="assistant") return false;
    const t=normSearch(x?.text||"");
    return (
      t.includes("bao hanh") ||
      t.includes("kiem tra bh") ||
      t.includes("tra cuu bh") ||
      (t.includes("so dien thoai") && (t.includes("kiem tra")||t.includes("tra cuu")))
    );
  });

  if(assistantRecentlyAskedWarranty && extractVietnamPhone(message)) return true;
