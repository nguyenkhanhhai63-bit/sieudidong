...
function directInstallmentAnswer(settings={}){
  ...
  const providerText=providers.length
    ? ` Hiện shop có cấu hình hỗ trợ qua ${providers.join(" và ")}.`
    : "";
  return `Có bạn nha. Siêu Di Động có hỗ trợ trả góp qua công ty tài chính.${providerText} Hồ sơ, số tiền trả trước, kỳ hạn và khoản góp sẽ tùy kết quả duyệt thực tế. Bạn đang muốn trả góp mẫu máy nào, mình kiểm tra giá máy rồi tư vấn tiếp cho bạn.`;
}
...
export default async function handler(req,res){
  ...
  // Trả góp là chính sách xác định từ cấu hình quản trị, xử lý trực tiếp để AI không bịa.
  ...
  const systemInstruction=[
    ...
    "PHONG CÁCH CHAT BẮT BUỘC: nhắn như một nhân viên thật đang chat, mỗi câu hoặc mỗi ý ngắn là một tin nhắn riêng. Không viết một đoạn văn dài.",
    ...
    try{
      ...
      if(r.ok){
        ...
        if(text){
          ...
          return res.status(200).json({ok:true,text:visible,model,needsHuman:false,knowledgeGap:!!parsed.missing});
        }
      }
