...
async function loadChatSettings(){
  ...
  try{
    ...
    let chatOnly={};
    try{
      chatOnly=redisHashObject(await redisCommand(["HGETALL","ai:chat:settings:v2"]));
    }catch(_){}
    if(!raw && !Object.keys(chatOnly).length) return defaults;
    const chat=chatOnly;

    return {
      chatInstructions:clean(chat.chatInstructions??x.chatInstructions,4000)||defaults.chatInstructions,
      chatWelcomeMessage:clean(chat.chatWelcomeMessage??x.chatWelcomeMessage,500)||defaults.chatWelcomeMessage,
      chatStoreFacts:clean(chat.chatStoreFacts??x.chatStoreFacts,2500)||defaults.chatStoreFacts,
      chatHandoffRules:clean(chat.chatHandoffRules??x.chatHandoffRules,1500)||defaults.chatHandoffRules,
      chatSuggestions:clean(chat.chatSuggestions??x.chatSuggestions,4000)||defaults.chatSuggestions
    };
  }catch(_){
    return defaults;
  }
