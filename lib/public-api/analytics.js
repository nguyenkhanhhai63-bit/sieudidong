...
export default async function handler(req,res){
  ...
  try{
    ...
    if(type==="page_view"){
      ...
      let countVisit=true;
      if(vid){
        const visitKey=`analytics:v3:visit_seen:day:${day}`;
      ...
      if(vid){
        // V276: SET chính xác theo ngày. Khách đã vào sẽ được giữ đến hết thời gian retention,
        // hoàn toàn độc lập với trạng thái Online.
        await redisCommand(["SADD","analytics:v4:visitors:all",vid]);
        await redisCommand(["SADD",`analytics:v4:visitors:day:${day}`,vid]); await exp(`analytics:v4:visitors:day:${day}`);

        // Giữ HLL cũ để tương thích dữ liệu lịch sử.
        await redisCommand(["PFADD","analytics:v3:visitors:all",vid]);
        await redisCommand(["PFADD",`analytics:v3:visitors:day:${day}`,vid]); await exp(`analytics:v3:visitors:day:${day}`);
    ...
    if(type==="heartbeat" && vid){
      ...
      // V276: heartbeat cũng ghi visitor vào SET theo ngày. SET này KHÔNG bị xóa khi offline.
      await redisCommand(["SADD","analytics:v4:visitors:all",vid]);
      await redisCommand(["SADD",`analytics:v4:visitors:day:${day}`,vid]); await exp(`analytics:v4:visitors:day:${day}`);

      // HLL cũ chỉ giữ để tương thích.
      await redisCommand(["PFADD","analytics:v3:visitors:all",vid]);
      await redisCommand(["PFADD",`analytics:v3:visitors:day:${day}`,vid]); await exp(`analytics:v3:visitors:day:${day}`);

      // Nếu page_view không tới server nhưng heartbeat tới, vẫn phải có ít nhất
      // một lượt truy cập trong ngày cho visitor đang online. Không cộng trùng
      // nếu page_view đã đánh dấu V3 trước đó.
      const seenKey=`analytics:v3:visit_seen:day:${day}`;
      const firstSeen=Number(await redisCommand(["SADD",seenKey,vid])||0);
      await exp(seenKey);
      if(firstSeen===1){
        await redisCommand(["INCR","analytics:raw_pageviews:all"]);
        await redisCommand(["INCR",`analytics:raw_pageviews:day:${day}`]); await exp(`analytics:raw_pageviews:day:${day}`);
