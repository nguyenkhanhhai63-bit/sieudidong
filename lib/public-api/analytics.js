...
// V843: "Lượt truy cập" = session 30 phút, không còn là mỗi page load.
// Mỗi hoạt động của cùng visitor kéo dài session hiện tại; sau 30 phút im lặng mới tính session mới.
const SESSION_TIMEOUT_MS=30*60*1000;
async function touchVisitSession(day,vid,now,trafficSource,visitHour){
  if(!vid) return false;
  const lastKey=`analytics:v6:session_last:${vid}`;
  const dayKey=`analytics:v5:day:${day}`;
  const sourceKey=`analytics:v6:traffic_sources:${day}`;
  const hourKey=`analytics:v6:traffic_hours:${day}`;
  const script=`
    local last=redis.call('GET',KEYS[1])
    local now=tonumber(ARGV[1])
    local timeout=tonumber(ARGV[2])
    local isNew=(not last) or ((now-tonumber(last))>=timeout)
    redis.call('SET',KEYS[1],ARGV[1],'PX',ARGV[3])
    if isNew then
      redis.call('HINCRBY',KEYS[2],'sessions',1)
      redis.call('EXPIRE',KEYS[2],ARGV[4])
      redis.call('INCR','analytics:v6:sessions:all')
      redis.call('ZINCRBY',KEYS[3],1,ARGV[5])
      redis.call('EXPIRE',KEYS[3],ARGV[4])
      redis.call('ZINCRBY','analytics:v6:traffic_sources:all',1,ARGV[5])
      redis.call('ZINCRBY',KEYS[4],1,ARGV[6])
  ...
  try{
    const r=await redisCommand([
      "EVAL",script,"4",lastKey,dayKey,sourceKey,hourKey,
      String(now),String(SESSION_TIMEOUT_MS),String(SESSION_TIMEOUT_MS+60*1000),String(RETENTION),trafficSource,visitHour
    ]);
    return Number(r||0)===1;
...
export default async function handler(req,res){
  ...
  try{
    ...
    const trafficSourceRaw=clean(req.body?.trafficSource,60);
    const trafficSource=["Trực tiếp","Google","Facebook","TikTok","Zalo","Khác"].includes(trafficSourceRaw) ? trafficSourceRaw : "Khác";
    const day=vnDay();
    const now=Date.now();
    // Giờ địa phương Việt Nam (0-23), dùng cho heatmap khung giờ truy cập.
    const vnNow=new Date(now+7*60*60*1000);
    const visitHour=String(vnNow.getUTCHours()).padStart(2,"0");
    ...
    // V843: page_view mở/duy trì session; heartbeat giữ session khi khách còn hoạt động.
    // Không để event phụ (device_enrich, filter...) vô tình tạo session nguồn "Khác" trước page_view.
    if(vid && (type==="page_view" || type==="heartbeat")) await touchVisitSession(day,vid,now,trafficSource,visitHour);

    if(type==="page_view"){
      // V843: page_view chỉ còn là "Lượt xem trang".
      await dayIncr(day,"pageviews",1);
      await redisCommand(["INCR","analytics:v5:pageviews:all"]);

      // Giữ key cũ để tương thích heartbeat/diagnostics cũ.
    ...
    if(type==="heartbeat" && vid){
      ...
      // Session đã được touch phía trên. Heartbeat không tự tạo page_view giả nữa.
    }
    ...
    if((type==="ai_chat_open" || type==="ai_chat_question") && vid){
      ...
      if(type==="ai_chat_question"){
        ...
        // V419 fallback: luôn ghi nhận câu khách hỏi. Khi /api/ai-chat trả lời thành công,
        // server sẽ ghi thêm bản đầy đủ có cả question + answer. Nhờ vậy không còn mất trắng lịch sử.
        if(question){
          const historyKey=`analytics:v5:ai_question_history:${day}`;
          const item=JSON.stringify({ts:now,visitorId:vid,question,answer:"",action:action||"ai_chat_question_pending",pending:true});
