...
export default async function handler(req,res){
  ...
  try{
    ...
    if(type==="page_view"){
      ...
      // Nguồn truy cập: đếm theo lượt tải trang để tổng nguồn khớp với Lượt truy cập.
      await redisCommand(["ZINCRBY",`analytics:v5:traffic_sources:${day}`,"1",trafficSource]);
      await expireKey(`analytics:v5:traffic_sources:${day}`);
      await redisCommand(["ZINCRBY","analytics:v5:traffic_sources:all","1",trafficSource]);

      // V508: thống kê lượt truy cập theo từng giờ trong ngày.
      const hourlyKey=`analytics:v5:traffic_hours:${day}`;
      await redisCommand(["ZINCRBY",hourlyKey,"1",visitHour]);
      await expireKey(hourlyKey);
