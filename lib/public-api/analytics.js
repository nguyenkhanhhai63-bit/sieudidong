...
export default async function handler(req,res){
  ...
  try{
    ...
    const deviceBrand=clean(req.body?.deviceBrand,100);
    const deviceModel=clean(req.body?.deviceModel,120);
    const deviceOs=clean(req.body?.deviceOs,120);
    const day=vnDay();
    ...
    if(type==="page_view"){
      ...
      if(vid){
        ...
        // Mỗi visitor chỉ đóng góp 1 lần vào thống kê model thiết bị.
        const modelLabel=deviceModel || (device==="mobile" ? "Điện thoại (không xác định model)" : "");
        if(modelLabel){
          const isNewModel=await redisCommand(["SADD","analytics:device_model_visitors:all",vid]);
          if(Number(isNewModel)===1){
            const label=deviceBrand && !modelLabel.toLowerCase().includes(deviceBrand.toLowerCase())
              ...
              : modelLabel;
            await redisCommand(["ZINCRBY","analytics:device_models:all","1",label]);
            if(deviceOs) await redisCommand(["ZINCRBY","analytics:device_os:all","1",deviceOs]);
          }
        }
      }
    }
