...
export default async function handler(req,res){
  ...
  try{
    if(req.method==="GET"){
      ...
      for(const id of (Array.isArray(ids)?ids:[])){
        ...
        rows.push({
          ...
          page:item.page||"/",
          meta:item.meta||{},
          visitorId:item.visitorId||item.meta?.visitorId||"",
          rating:item.rating||"",
    ...
    if(req.method==="PATCH"){
      const id=clean(req.body?.id,90);
      const rating=clean(req.body?.rating,20);
      if(!id || !["good","bad",""].includes(rating)) return res.status(400).json({error:"Dữ liệu đánh giá không hợp lệ"});
      const raw=await redisGet(PREFIX+id);
      const item=safeParse(raw);
      if(!item) return res.status(404).json({error:"Không tìm thấy hội thoại"});
      item.rating=rating; item.ratedAt=Date.now();
      await redisCommand(["SET",PREFIX+id,JSON.stringify(item),"EX",String(30*24*60*60)]);
