...
import { fetchUsedSheet, mergeSheetWithOverlays, getUsedSheetConfig } from "../used-products-sheet.js";
...
function normalize(x={}){
  ...
  return {
    ...
    source:x.source==="sheet"?"sheet":"manual",sheetRow:Math.max(0,Number(x.sheetRow||0)||0),
    name:clean(x.name,140),brand:clean(x.brand,60),memory:clean(x.memory,60),color:clean(x.color,60),rom:clean(x.rom,80),
...
export default async function handler(req,res){
  ...
  if(req.method==="GET") {
    try{
      const [sheet, overlays, cfg]=await Promise.all([fetchUsedSheet({force:String(req.query?.force||"")==="1"}), readOverlays(), getUsedSheetConfig()]);
      const items=mergeSheetWithOverlays(sheet.items,overlays);
      return res.status(200).json({ok:true,items,source:"google-sheet",sheet:{id:cfg.id,gid:cfg.gid,url:cfg.url,custom:!!cfg.custom,fetchedAt:sheet.fetchedAt,cached:!!sheet.cached,warning:sheet.warning||""}});
    }catch(error){
