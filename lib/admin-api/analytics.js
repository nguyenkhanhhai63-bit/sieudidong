...
export default async function handler(req,res){
  ...
  try{
    ...
    const [
      ...
      pf([`analytics:visitors:day:${yesterday}`]),
      ...
      (async()=>{
        ...
        return num(await safeCommand(["ZCOUNT","analytics:online",String(now-5*60*1000),"+inf"],0));
      })()
    ]);
    ...
    const visitors7=await pf(d7.map(d=>`analytics:visitors:day:${d}`));
    const visitors30=await pf(d30.map(d=>`analytics:visitors:day:${d}`));
    ...
    const visitorsFor=ds=>pf(ds.map(d=>`analytics:visitors:day:${d}`));
