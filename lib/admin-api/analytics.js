...
async function hourlyHeatmapAcross(days){
  // V855: mỗi hàng là MỘT NGÀY THỰC TẾ, không cộng dồn theo thứ trong tuần.
  // Dashboard truyền đúng 7 ngày gần nhất để heatmap luôn dễ đọc và so sánh.
  const safeDays=(Array.isArray(days)?days:[]).slice(-7);
  const matrix=Array.from({length:safeDays.length},()=>Array(24).fill(0));
  if(!safeDays.length)return matrix;
  const rowByDay=new Map(safeDays.map((d,i)=>[String(d),i]));
  const script=`
    local out={}
    for _,day in ipairs(KEYS) do
      local z=redis.call('ZRANGE','analytics:v6:traffic_hours:'..day,0,-1,'WITHSCORES')
      for i=1,#z,2 do
    ...
    return out
  `;
  const raw=await cmd(["EVAL",script,String(safeDays.length),...safeDays],[],10000);
...
async function readDays(days){
  ...
  if(!Array.isArray(raw)) return days.map(date=>({date,visitors:0,views:0,pageviews:0,rawViews:0,detailClicks:0,compare:0,zalo:0,aiOpens:0,aiQuestions:0,warrantyChecks:0,warrantyFound:0,warrantyNotFound:0,warrantyErrors:0}));
  const rows=[];
  for(let i=0;i<raw.length;i+=3){
    ...
    rows.push({date,visitors,views:num(h.sessions),pageviews:num(h.pageviews),rawViews:num(h.pageviews),detailClicks:num(h.detailClicks),compare:num(h.compare),zalo:num(h.zalo),aiOpens:num(h.aiOpens),aiQuestions:num(h.aiQuestions),warrantyChecks:num(h.warrantyChecks),warrantyFound:num(h.warrantyFound),warrantyNotFound:num(h.warrantyNotFound),warrantyErrors:num(h.warrantyErrors)});
  }
  return rows;
}
...
function period(rows,visitors,warrantyPhones=0,aiUsers=0){return{visitors:num(visitors),views:sum(rows,"views"),pageviews:sum(rows,"pageviews"),detailClicks:sum(rows,"detailClicks"),compare:sum(rows,"compare"),zalo:sum(rows,"zalo"),aiUsers:num(aiUsers),aiOpens:sum(rows,"aiOpens"),aiQuestions:sum(rows,"aiQuestions"),warrantyChecks:sum(rows,"warrantyChecks"),warrantyFound:sum(rows,"warrantyFound"),warrantyPhones:num(warrantyPhones)}}
...
export default async function handler(req,res){
  ...
  try{
    ...
    const daily=d30.map(d=>rowMap.get(d)||{date:d,visitors:0,views:0,pageviews:0,rawViews:0,detailClicks:0,compare:0,zalo:0,aiOpens:0,aiQuestions:0,warrantyChecks:0,warrantyFound:0,warrantyNotFound:0,warrantyErrors:0});
    ...
    const [vis7,vis30,visMonth,visYear,phonesToday,phones7,phones30,phonesMonth,phonesYear,totalVisitors,totalSessions,totalPageviews,devices,topProducts,topSearches,topCompareProducts,topComparePairs,modelsV2,modelsOld,osV2,osOld,filters,warrantyPhonesAll]=await Promise.all([
      ...
      zcard("analytics:v5:visitors:all"),getNum("analytics:v6:sessions:all"),getNum("analytics:v5:pageviews:all"),hgetall("analytics:devices:all"),
      ...
      hgetall("analytics:filters:all"),num(await cmd(["PFCOUNT","analytics:warranty_phones:all"],0,5000))
    ]);
    ...
    const [productRanks,searchRanks,compareProductRanks,comparePairRanks,devicePeriod,aiQuestionHistory,trafficSources,hourlyHeatmap]=await Promise.all([
      ...
      topAcross("analytics:v6:traffic_sources:",requestedDays,10),
      hourlyHeatmapAcross(d7)
    ]);
    ...
    const rankingsByPeriod={
      [requestedPeriod]:{
        ...
        trafficSources,
        hourlyHeatmap,
        hourlyHeatmapDays:d7
      }
    };
    const monthlyMap={};
    for(const x of rowsYear){const m=x.date.slice(0,7);if(!monthlyMap[m])monthlyMap[m]={month:m,views:0,pageviews:0,detailClicks:0,compare:0,zalo:0,aiOpens:0,aiQuestions:0,warrantyChecks:0,warrantyFound:0};for(const k of ["views","pageviews","detailClicks","compare","zalo","aiOpens","aiQuestions","warrantyChecks","warrantyFound"])monthlyMap[m][k]+=num(x[k])}
    const p30=period(daily,vis30,phones30), p7=period(rows7,vis7,phones7);
    return res.status(200).json({ok:true,analyticsVersion:"v843-sessions",generatedAt:new Date().toISOString(),analyticsDay:today,timezone:"Asia/Ho_Chi_Minh",periods,rankingsByPeriod,history:rowsYear,daily,monthly:Object.values(monthlyMap).slice(-12),overview:{todayVisitors:todayRow.visitors,todayViews:todayRow.views,onlineNow:active.length,totalVisitors,totalViews:totalSessions,totalSessions,totalPageviews,totalRawViews:totalPageviews,views7:p7.views,views30:p30.views,pageviews7:p7.pageviews,pageviews30:p30.pageviews,todayPageviews:todayRow.pageviews,visitors30:vis30,detailClicks:await getNum("analytics:v5:detailClicks:all"),compareAll:await getNum("analytics:v5:compare:all"),zaloAll:await getNum("analytics:v5:zalo:all"),todayCompare:todayRow.compare,compare30:p30.compare,todayZalo:todayRow.zalo,zalo30:p30.zalo,todayAiUsers:aiToday,aiUsers30:ai30,todayAiOpens:todayRow.aiOpens,aiOpens30:sum(daily,"aiOpens"),todayAiQuestions:todayRow.aiQuestions,aiQuestions30:sum(daily,"aiQuestions"),aiUsersAll:await zcard("analytics:v5:ai_users:all"),aiQuestionsAll:await getNum("analytics:v5:ai_questions:all"),todayWarrantyChecks:todayRow.warrantyChecks,warrantyChecks7:p7.warrantyChecks,warrantyChecks30:p30.warrantyChecks,warrantyChecksAll:sum(rowsYear,"warrantyChecks"),warrantyPhonesToday:phonesToday,warrantyPhones30:phones30,warrantyPhonesAll,todayWarrantyFound:todayRow.warrantyFound,warrantyFound30:p30.warrantyFound},devices,topProducts,topSearches,topCompareProducts,topComparePairs,topDeviceModels:modelsV2.length?modelsV2:modelsOld,topDeviceOs:osV2.length?osV2:osOld,filters,diagnostics:{model:"V843-SESSION-30M",todayVisitors:todayRow.visitors,todayViews:todayRow.views,todayPageviews:todayRow.pageviews,onlineNow:active.length,daysRead:rowsYear.length}});
