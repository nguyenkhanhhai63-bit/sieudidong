...
async function scard(key){ return num(await safeCommand(["SCARD",key],0,8000)); }
async function exactVisitorsForDays(days=[]){
  const unique=new Set();
  for(const d of days){
    const ids=await safeCommand(["SMEMBERS",`analytics:v4:visitors:day:${d}`],[],8000);
    if(Array.isArray(ids)) ids.forEach(id=>unique.add(String(id)));
  }
  return unique.size;
}
...
export default async function handler(req,res){
  ...
  try{
    ...
    /* V271: Online phải được reconcile TRƯỚC khi đọc số Hôm nay.
       V270 chạy việc PFADD online visitor và PFCOUNT Hôm nay trong cùng Promise.all,
       nên PFCOUNT có thể trả 0 trước khi PFADD hoàn tất. */
    const now=Date.now();
    const onlineMin=String(now-5*60*1000);
    await safeCommand(["ZREMRANGEBYSCORE","analytics:online","0",onlineMin],0,8000);
    const activeOnlineRaw=await safeCommand(
      ["ZRANGEBYSCORE","analytics:online",onlineMin,"+inf"],
      [],
      8000
    );
    const activeOnlineIds=[...new Set(
      (Array.isArray(activeOnlineRaw)?activeOnlineRaw:[]).map(String).filter(Boolean)
    )];

    if(activeOnlineIds.length){
      /* V276: visitor online được cộng vào SET tích lũy hôm nay TRƯỚC khi đọc số.
         Sau khi visitor offline, SET này vẫn giữ nguyên. */
      await safeCommand(["SADD","analytics:v4:visitors:all",...activeOnlineIds],0,8000);
      await safeCommand(["SADD",`analytics:v4:visitors:day:${today}`,...activeOnlineIds],0,8000);
      await safeCommand(["EXPIRE",`analytics:v4:visitors:day:${today}`,String(400*24*60*60)],0,8000);

      /* HLL cũ giữ để tương thích. */
      await safeCommand(["PFADD","analytics:v3:visitors:all",...activeOnlineIds],0,8000);
      await safeCommand(["PFADD",`analytics:v3:visitors:day:${today}`,...activeOnlineIds],0,8000);
      await safeCommand(["EXPIRE",`analytics:v3:visitors:day:${today}`,String(400*24*60*60)],0,8000);

      /* Mỗi visitor online tối thiểu phải tạo 1 lượt truy cập hôm nay.
         SADD giúp không cộng trùng với page_view/heartbeat đã ghi trước đó. */
      const seenKey=`analytics:v3:visit_seen:day:${today}`;
      for(const vid of activeOnlineIds){
        const added=num(await safeCommand(["SADD",seenKey,vid],0,8000));
        if(added===1){
          await safeCommand(["INCR","analytics:raw_pageviews:all"],0,8000);
          await safeCommand(["INCR",`analytics:raw_pageviews:day:${today}`],0,8000);
        }
      }
      await safeCommand(["EXPIRE",seenKey,String(400*24*60*60)],0,8000);
      await safeCommand(["EXPIRE",`analytics:raw_pageviews:day:${today}`,String(400*24*60*60)],0,8000);
    }

    const onlineNow=activeOnlineIds.length;

    /* Sau khi reconcile xong mới đọc Hôm nay/Hôm qua. */
    const [
      ...
      topDeviceModels,topDeviceOs,filters,warrantyRecent
    ]=await Promise.all([
      getNum(`analytics:raw_pageviews:day:${today}`),
      getNum(`analytics:raw_pageviews:day:${yesterday}`),
      scard(`analytics:v4:visitors:day:${today}`),
      scard(`analytics:v4:visitors:day:${yesterday}`),
      ...
      scard("analytics:v4:visitors:all"),
    ...
    /* Safety invariant: đang online > 0 thì visitor/visit hôm nay không thể là 0.
       Giá trị này chủ yếu bảo vệ UI trong trường hợp Redis eventual timing bất thường. */
    const safeTodayVisitors=Math.max(num(todayVisitors),onlineNow);
    const safeTodayVisits=Math.max(num(todayVisits),onlineNow);
    ...
    const visitorsYear = yearDates.length<=120 ? await visitorsFor(yearDates) : await scard("analytics:v4:visitors:all");
    ...
    const periods={
      ...
      today:{views:safeTodayVisits,visitors:safeTodayVisitors,zalo:todayZalo,compare:todayCompare,detailClicks:todayDetail,warrantyChecks:todayWarranty,warrantyFound:todayWarrantyFound,warrantyPhones:warrantyPhonesToday},
    ...
    return res.status(200).json({
      ...
      retentionDays:400,
      overview:{
        todayViews:safeTodayVisits,
        views7:periods.days7.views,
        views30:periods.days30.views,
        totalViews:totalRawViews,
        todayVisitors:safeTodayVisitors,visitors7,visitors30,totalVisitors,
        ...
        todayRawViews:safeTodayVisits,totalRawViews,
      ...
      diagnostics:{
        ...
        todayVisits:safeTodayVisits,
        todayVisitors:safeTodayVisitors,
        redisTodayVisits:todayVisits,
        redisTodayVisitors:todayVisitors,
        onlineNow,
        reconciledOnlineIds:activeOnlineIds.length
      }
    });
  }catch(err){
