...
export async function lookupWarrantyByPhone(phoneInput){
  ...
  if(!exact.length){
    return {
      ok:true,found:false,customerName:"",items:[],
      message:"Không tìm thấy lịch sử mua hàng với số điện thoại này."
    };
  }

  const customerName=String(
    exact.find(c=>c && (c.name||c.customerName||c.fullName))?.name ||
    exact.find(c=>c && (c.name||c.customerName||c.fullName))?.customerName ||
    exact.find(c=>c && (c.name||c.customerName||c.fullName))?.fullName ||
    ""
  ).trim();
